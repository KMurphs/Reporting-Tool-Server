  ...
  <div class="autocomplete"> <!--  v-bind:style="configStyle"> -->
    ...
    <div v-bind:id="inputID + 'autocomplete-list'"
         ...
         <div v-on:click="onSelection(item)"
              v-for="(item, index) in configData.filter((item) => { return (item.substr(0, this.currentInput.length).toUpperCase() == this.currentInput.toUpperCase()) })"
              v-bind:key="index"
              v-bind:ref="'item-' + index"
              v-bind:class="{
                  'autocomplete-active': (index == currentFocus),
                }">
              <strong>{{ item.substr(0, currentInput.length) }}</strong>{{item.substr(currentInput.length)}}
...
export default {
    ...
    methods: {
      onInput: function(){
        ...
        this.matchingData = this.configData.filter((item) => { return (item.substr(0, this.currentInput.length).toUpperCase() == this.currentInput.toUpperCase()) })
      ...
      scrollAutoCompleteItems: function(itemIndex, direction){
        ...
        let autocompleteitems = this.$refs.autocompleteitems;
        let currentElemt = this.$refs['item-' + itemIndex];
        ...
        let elemtHeight = currentElemt[0].scrollHeight + 1;
        let reqHeightStart = itemIndex * elemtHeight;
        let reqHeightEnd = (itemIndex + 1) * elemtHeight;
      ...
      onKeyDown: function(event){
        if (event.keyCode == 40) {
          ...
          if (this.currentFocus > -1) {
            ...
            this.onSelection(this.configData.filter((item) => { return (item.substr(0, this.currentInput.length).toUpperCase() == this.currentInput.toUpperCase()) })[this.currentFocus])
          }
        }
      },
